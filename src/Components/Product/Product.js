...
export default class Product extends Component {
  ...
  async delete() {
    ...
    if (c) {
      const res = await axios.delete(`/api/product/${this.props.product.id}`).catch(err => {
        console.log(err)
  ...
  async submitEdit() {
    const { sName, sPrice, sImg } = this.state,
      ...
      res = await axios.put(`/api/product/${id}`, body).catch(err => console.log(err))
    this.props.setInventory(res.data)
    this.setState({ editing: false })
  }
  ...
  render() {
    ...
    return (
      <div className='prod-card' onMouseEnter={this.hoverOn} onMouseLeave={this.hoverOff}>
